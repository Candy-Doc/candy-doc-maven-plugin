...
public class InteractionChecker {
  ...
  public List<io.candydoc.domain.events.DomainEvent> check(CheckConceptInteractions command) {
    ...
    ProcessorUtils.getInstance().getMessager().printMessage(Diagnostic.Kind.NOTE, "Checking concept " + conceptClass);
    AnnotationMirror dddAnnotation = conceptTypeFor(conceptClass);
    ProcessorUtils.getInstance().getMessager().printMessage(Diagnostic.Kind.NOTE, "Found DDD annotation in it: " + dddAnnotation);
    InteractionStrategy strategy = strategyFor(dddAnnotation);
    ProcessorUtils.getInstance().getMessager().printMessage(Diagnostic.Kind.NOTE, "Strategy: " + strategy);
    return strategy.checkInteractions(conceptClass);
  }

  private AnnotationMirror conceptTypeFor(TypeElement concept) {
    List<AnnotationMirror> annotationMirrors = concept.getAnnotationMirrors().stream()
        .filter(InteractionChecker::dddAnnotationOnly)
        .collect(Collectors.toList());
    if (annotationMirrors.isEmpty())
      return conceptTypeForSuperClassOf(concept);
    else
  ...
  private static Boolean dddAnnotationOnly(AnnotationMirror annotation) {
    return InteractionStrategy.DDD_ANNOTATION_CLASSES.stream().anyMatch(
        annotationType -> annotationType.getCanonicalName().equals(annotation.getAnnotationType().toString())
    );
  }
  ...
  private InteractionStrategy strategyFor(AnnotationMirror annotation) {
    return interactionStrategies.entrySet().stream()
        .filter(
            entry -> entry.getKey().getCanonicalName().equals(annotation.getAnnotationType().toString())
        )
        .findFirst()
        .orElseThrow(() -> new ElementNotAnnotatedByDDDConcept(annotation.getAnnotationType().asElement()))
        .getValue();
  }
}
